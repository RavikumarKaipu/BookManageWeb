...
const AddEditBook = () => {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://bookmanage-backend.vercel.app/books/${id}`)
        .then(response => setBook(response.data))
        .catch(error => console.log(error));
    }
  }, [id]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const request = id 
      ? axios.put(`http://bookmanage-backend.vercel.app/books/${id}`, book) 
      : axios.post(`http://bookmanage-backend.vercel.app/books`, book);
